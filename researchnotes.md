...
| **SP4** | Parallel cheap probes | **H-EE-007** labels; optional H-EE-002 under hybrid | untested | Label/gain causality yes/no | Not the main residual program |
...
| 2 | H-EE-007 / SP4 | raw `labels` vs `policy_labels` probe | Optional 1-seed | Low | EE label noise |
...
1. **SP4 / H-EE-007** — label-contract probe; prompt: `prompts/h-ee-007-label-contract-probe.md`.
2. **SP4 / H-EE-002** — frozen-hybrid gain causality; prompt: `prompts/h-ee-002-hybrid-gain-sweep.md`.
3. **SP2b / H-EE-015** — oracle FSM arm upper bound; prompt: `prompts/h-ee-015-fsm-arm-upper-bound.md`.
4. **H-EE-017** — history / tiny GRU only if the decisive tests leave a non-Markov residual.
5. **SP3 train path** — only if softer close/approach design is registered (not gate relaxation).
6. **SP6** joint pick-place track optional (joint hybrid 97/120 still best).
7. **SP5 final** — still closed; EE 62/worst 9/phys 68 short of legacy_84.
8. **SP7** Phase 6b blocked; Phase 7 language/VLA is not the next step.
9. **Crossed off:** H-EE-022 match-set, H-EE-023 A2 arm-only, H-EE-016/018/019, pure gripper MSE, distance guards.
10. **Keep frozen:** hybrid A1 + `global_gripper` + historical match + H-EE-008/021 foundations.
