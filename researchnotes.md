...
1. **Demonstration efficiency** — preregistered nested/stratified demo-count curve, both spaces, common seeds. **Status: `registered_not_run`, independently reviewed and ready for the complete development curve (EFF-001).**
...
| **Claim** | Under the frozen hybrid A1 fair contract, joint-delta and ee_tool_delta differ in sample efficiency as measured by normalized AUC of success rate versus distinct demo count (budgets 6/12/18/24/30), with crossed-factor paired bootstrap uncertainty over ladders and model seeds. |
...
| **Uncertainty** | Crossed-factor paired bootstrap: resample ladders and model seeds independently; cells sharing either factor are not independent |
...
| **Review** | Passed 2026-07-15 after mode-isolation, resume-provenance, exposure-rate, and crossed-bootstrap corrections |
| **Test** | Next: complete `--mode primary-curve` on development split; locked only with separate authorization |
| **Result** | *Not run — ready for the complete development primary curve.* |
...
Primary command (review passed; run on a focused research branch):
...
|-----:|----|------|----------|-------|
| 1 | EFF-001 | Nested stratified demo-count curve | Yes | **`registered_not_run`** — independent review passed; complete development primary curve is next |
...
| 2026-07-14/15 | EFF-001 | Efficiency-curve infrastructure + preregistration; independent review fixed mode isolation, resume identity, exposure-rate math, and crossed-factor uncertainty | **`registered_not_run`** — READY_FOR_PRIMARY_CURVE; primary curve and locked evaluation **not** executed | `evidence/state_bc_efficiency_curve_registration.json`, `configs/state_bc_efficiency_protocol_v1.json`, `reports/2026-07-14-efficiency-curve-preregistration.md` |
